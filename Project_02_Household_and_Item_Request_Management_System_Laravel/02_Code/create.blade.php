...
<div class="premium-card shadow-xl p-8 mt-6">
    ...
    <form action="{{ route('user.pengajuan.store') }}" method="POST" class="grid gap-6 md:grid-cols-2">
        @csrf
        ...
        <div class="relative w-full group-input-glass"
            x-data="{ 
                    open: false, 
                    search: '',
                    selectedName: '{{ old('new_stock_name') ? old('new_stock_name') : (old('stock_id') ? $stock->firstWhere('id', old('stock_id'))?->nama_barang : '-- Pilih Barang --') }}', 
                    selectedValue: '{{ old('stock_id') }}',
                    newStockName: '{{ old('new_stock_name') }}',
                    isNew: {{ old('is_new_item') ? 'true' : 'false' }},
                    stocks: [
                        @foreach($stock as $item)
                        { id: '{{ $item->id }}', nama: '{{ addslashes($item->nama_barang) }}' },
                        @endforeach
                    ],
                    get filteredStocks() {
            ...
            <div x-show="isNew"
                ...
                <div x-show="isNew"
                    ...
                    x-transition:leave-start="opacity-100 translate-y-0 max-h-40"
                    x-transition:leave-end="opacity-0 -translate-y-4 max-h-0"
                    {{-- DI SINI: overflow-hidden dihapus agar list dropdown tidak terpotong ke dalam --}}
                    class="mt-4 p-4 rounded-xl bg-amber-50/40 border border-dashed border-amber-200 relative"
                    {{-- DI SINI: z-index dinaikkan secara dinamis saat dropdown satuan terbuka agar berada di atas input Jumlah --}}
                    :class="openSatuan ? 'z-40' : 'z-10'">

                    <div class="relative w-full" x-data="{ openSatuan: false, selectedSatuan: '{{ old('satuan', $satuan[0] ?? 'Pcs') }}' }">
                        ...
                        {{-- PERBAIKAN: Menggunakan z-[99] dan shadow yang lebih tebal agar melayang sempurna --}}
                        <div x-show="openSatuan"
                            ...
                            <div class="p-1">
                                {{-- PERBAIKAN: Mengubah nama alias agar tidak bentrok ($satuan sebagai $item) --}}
                                @foreach($satuan as $item)
                                <div @click="selectedSatuan = '{{ $item }}'; openSatuan = false"
                                    ...
                                    :class="selectedSatuan == '{{ $item }}' ? 'bg-amber-50 text-amber-800 font-bold' : ''">
                                    {{ $item }}
                                </div>
                                @endforeach
                            </div>
        ...
        <div class="group-input-glass">
            ...
            <div class="relative">
                ...
                <input type="number" name="jumlah_pengajuan"
                    ...
                    value="{{ old('jumlah_pengajuan') }}" placeholder="Contoh: 10" required>
            </div>
        </div>

        <div class="relative w-full group-input-glass" x-data="{ open: false, selectedName: '{{ old('prioritas') ? old('prioritas') : '-- Pilih Prioritas --' }}', selectedValue: '{{ old('prioritas') }}' }">
            <label class="block text-xs font-bold uppercase tracking-wider text-[#092540] mb-2 opacity-75">
        ...
        <div class="md:col-span-2 group-input-glass">
            ...
            <textarea name="keterangan"
                ...
                rows="3" placeholder="Tulis alasan atau rincian spesifikasi barang jika dibutuhkan...">{{ old('keterangan') }}</textarea>
        </div>
