...
<div class="premium-card shadow-xl">
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 px-6 py-5 bg-gradient-to-r from-white to-gray-50">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <!-- Search Input -->
            <form id="filterForm" method="GET" action="{{ url()->current() }}" class="flex flex-col sm:flex-row gap-3">
                <div class="relative">
            ...
            <!-- <a href="{{ url('user/pengajuan/create') }}" class="btn-premium px-5 py-2.5 rounded-xl text-white font-semibold flex items-center gap-2 shadow-md">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                $labelStatus = 'Disetujui semua';
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500">
                        {{ method_exists($pengajuan, 'firstItem') ? $pengajuan->firstItem() + $key : $key + 1 }}
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <button type="button"
                            onclick="openModal('modalStatus{{ $item->id }}')"
                            class="inline-flex items-center gap-1">
                            @if($labelStatus === 'Disetujui semua')
                            <span class="badge-success cursor-pointer">✅ Disetujui semua</span>
                            @elseif($labelStatus === 'Disetujui sebagian')
                            <span class="badge-warning cursor-pointer">
                                ⚠️ Disetujui sebagian
                                <span class="text-xs">({{ $jumlahDisetujui }}/{{ $jumlahPengajuan }})</span>
                            </span>
                            @elseif($labelStatus === 'Ditolak')
                    ...
                    <div id="modalStatus{{ $item->id }}"
                        ...
                        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden">
                            <div class="bg-gradient-to-r from-[#0B3B5F] to-[#0A2E4A] px-6 py-5 flex items-center justify-between">
                                ...
                                <button type="button"
                                    onclick="closeModal('modalStatus{{ $item->id }}')"
                            ...
                            <div class="p-6 space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                                        ...
                                        <p class="text-sm font-bold text-gray-800 mt-1">
                                            @if(in_array($labelStatus, ['Disetujui semua', 'Disetujui sebagian']))
                                            {{ $jumlahDisetujui }} {{ $satuan }}
                                            @else
                                            -
                                            @endif
                                        </p>
                                ...
                                <div class="rounded-2xl border border-green-200 bg-green-50 p-5">
                                    ...
                                    <p class="text-sm text-green-700 mt-1">
                                        ...
                                        <strong>{{ $jumlahPengajuan }} {{ $satuan }}</strong>
                                        telah disetujui.
                                    </p>
                                </div>
                                @elseif($labelStatus === 'Disetujui sebagian')
                                <div class="rounded-2xl border border-yellow-200 bg-yellow-50 p-5">
                                    ...
                                    <p class="text-sm text-yellow-700 mt-1">
                                        ...
                                        <strong>{{ $jumlahPengajuan }} {{ $satuan }}</strong>,
                                        admin menyetujui
                                        <strong>{{ $jumlahDisetujui }} {{ $satuan }}</strong>.
                                    </p>
                                </div>
                                @elseif($labelStatus === 'Ditolak')
                                <div class="rounded-2xl border border-red-200 bg-red-50 p-5">
                                    ...
                                    <div class="mt-4 rounded-xl bg-white border border-red-100 p-4">
                                        ...
                                        <p class="text-sm text-gray-700 whitespace-pre-line">
                                            {{ $item->keterangan ?? 'Tidak ada keterangan penolakan.' }}
                                        </p>
                                    </div>
                                </div>
                                @else
