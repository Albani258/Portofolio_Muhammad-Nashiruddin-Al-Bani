...
@section('title', 'Dashboard Pengadaan Anda')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 px-6 py-5 bg-gradient-to-r from-white to-gray-50/50">
            ...
            <form method="GET" action="{{ route('admin.permintaan_user.index') }}" class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 font-medium text-gray-700 bg-white">
                    ...
                    <tr class="hover:bg-[#F5E6B8]/10 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($item->status_pengajuan == 'Pending' || empty($item->status_pengajuan))
                            ...
                            <span class="badge-warning">⚠ Sebagian ({{ $item->jumlah_disetujui }})</span>
                            @else
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('admin.permintaan_user.updateStatus', $item->id) }}" method="POST" onsubmit="return confirm('Setujui semua jumlah pengajuan ini?')">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form id="form-partial-{{ $item->id }}" action="{{ route('admin.permintaan_user.updateStatus', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="aksi_persetujuan" value="approve_partial">
                                    <input type="hidden" name="jumlah_disetujui" id="partial-qty-{{ $item->id }}">
                                    <button type="button" onclick="approvePartial({{ $item->id }}, {{ $item->jumlah_pengajuan }})" class="px-2.5 py-1.5 rounded-lg bg-amber-500 text-white text-xs font-bold hover:bg-amber-600 shadow-sm transition-all whitespace-nowrap">
                                        ⚠ Sebagian
                                    </button>
                                </form>

                                <form id="form-reject-{{ $item->id }}" action="{{ route('admin.permintaan_user.updateStatus', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="aksi_persetujuan" value="reject">
                                    <input type="hidden" name="alasan_penolakan" id="reject-reason-{{ $item->id }}">
                                    <button type="button" onclick="rejectPengajuan({{ $item->id }})" class="px-2.5 py-1.5 rounded-lg bg-red-600 text-white text-xs font-bold hover:bg-red-700 shadow-sm transition-all whitespace-nowrap">
                                        ✕ Tolak
                                    </button>
                                </form>
                            </div>
