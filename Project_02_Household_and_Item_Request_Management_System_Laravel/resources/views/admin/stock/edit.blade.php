...
<div class="premium-card shadow-2xl">
    ...
    <!-- Form -->
    <form action="{{ route('admin.stock.update', $stock->id) }}"
        method="POST"
        class="p-8">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <input
                    type="text"
                    name="kode_barang"
                    value="{{ old('kode_barang', $stock->kode_barang) }}"
                    required
                    class="w-full rounded-2xl border-2 border-gray-200 px-5 py-3 text-sm transition-all focus:border-[#D4A017] focus:outline-none focus:ring-4 focus:ring-[#D4A017]/10">
            </div>
            ...
            <div>
                ...
                <select
                    ...
                    @foreach($kategori as $item_kategori)
                        <option
                            value="{{ $item_kategori }}"
                            {{ old('kategori', $stock->kategori) == $item_kategori ? 'selected' : '' }}>
                            {{ $item_kategori }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input
                    type="number"
                    name="jumlah_stock"
                    value="{{ old('jumlah_stock', $stock->jumlah_stock) }}"
                    min="0"
                    required
                    class="w-full rounded-2xl border-2 border-gray-200 px-5 py-3 text-sm transition-all focus:border-[#D4A017] focus:outline-none focus:ring-4 focus:ring-[#D4A017]/10">
            </div>
            ...
            <div>
                ...
                <input
                    type="number"
                    name="minimal_stock"
                    value="{{ old('minimal_stock', $stock->minimal_stock) }}"
                    min="0"
                    required
                    class="w-full rounded-2xl border-2 border-gray-200 px-5 py-3 text-sm transition-all focus:border-[#D4A017] focus:outline-none focus:ring-4 focus:ring-[#D4A017]/10">
            </div>
            ...
            <div>
                ...
                <select
                    ...
                    @foreach($satuan as $item_satuan)
                        <option
                            value="{{ $item_satuan }}"
                            {{ old('satuan', $stock->satuan) == $item_satuan ? 'selected' : '' }}>
                            {{ $item_satuan }}
                        </option>
                    @endforeach
                </select>
