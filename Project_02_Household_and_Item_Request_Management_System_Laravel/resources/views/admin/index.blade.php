...
<div class="relative mb-8 overflow-hidden rounded-[28px] bg-gradient-to-br from-[#082F49] via-[#0B3B5F] to-[#123C5C] p-8 shadow-2xl">

    <!-- Decorative Glow -->
    <div class="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-[#D4A017]/20 blur-3xl"></div>
    <div class="absolute -bottom-20 left-1/3 h-48 w-48 rounded-full bg-white/10 blur-3xl"></div>

    <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">

        <!-- Left Content -->
        <div class="max-w-4xl">
            <div class="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-semibold text-[#F7D27A] backdrop-blur-sm">
                Dashboard Monitoring Rumah Tangga
            </div>

            <h1 class="text-2xl font-extrabold leading-snug text-white md:text-3xl">
                Selamat Datang di Dashboard
                <span class="text-[#F7D27A]">BPSDM Kemenimipas</span>
            </h1>

            <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-200/90">
                Kelola data stock barang, pengajuan, pengadaan, dan evaluasi kebutuhan secara terpusat, cepat, dan terukur.
            </p>
            </div>
        </div>
    </div>
...
<div class="stat-card-premium p-5">
    ...
    <div class="mt-2 w-full bg-gray-200 rounded-full h-1.5">
        <div class="bg-green-500 h-1.5 rounded-full"
            style="width: {{ $persenJumlahStock ?? 0 }}%">
        </div>
    </div>
</div>
...
<div class="stat-card-premium p-5">
    ...
    <div class="mt-2 w-full bg-gray-200 rounded-full h-1.5">
        <div class="bg-orange-500 h-1.5 rounded-full"
            style="width: {{ $persenPengajuanMasuk ?? 0 }}%">
        </div>
    </div>
</div>
    ...
    <div class="stat-card-premium p-5">
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-3xl font-bold text-purple-700" id="total_pengadaan">{{ $totalPengadaan ?? 0 }}</span>
        </div>
    ...
    <div class="stat-card-premium p-5">
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-3xl font-bold text-emerald-700" id="permintaan_aktif">{{ $permintaanAktif ?? 0 }}</span>
        </div>
        ...
        <div class="mt-2 flex items-center justify-between">
            ...
            <span class="text-sm font-semibold text-red-500" id="masih_diproses">{{ $masihDiproses ?? 0 }}</span>
        </div>
        <div class="mt-2 w-full bg-gray-200 rounded-full h-1.5">
            ...
            @forelse($pengajuPalingAktif as $index => $item)
                <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4 hover:bg-[#F5E6B8]/20 transition">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            <p class="font-bold text-gray-800">{{ $item->nama_pengaju ?? '-' }}</p>
                            <p class="text-xs text-gray-500">{{ $item->divisi_pengaju ?? '-' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="font-bold text-[#0B3B5F]">{{ $item->total_pengajuan ?? 0 }}</p>
        ...
        <div class="p-5 space-y-3">
            ...
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4 hover:bg-[#F5E6B8]/20 transition">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="font-bold text-gray-800">{{ $item->stock->nama_barang ?? '-' }}</p>
                        <p class="text-xs text-gray-500">
                            {{ $item->stock->kode_barang ?? '-' }} • {{ $item->stock->kategori ?? '-' }}
                        </p>
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            <div class="rounded-xl bg-white p-3 border border-gray-100">
                                ...
                                <p class="font-bold text-[#0B3B5F]">
                                    {{ $item->total_diminta ?? 0 }} {{ $item->stock->satuan ?? '' }}
                                </p>
                            </div>
                            ...
                            <div class="rounded-xl bg-white p-3 border border-gray-100">
                                ...
                                <p class="font-bold text-emerald-600">
                                    {{ $item->total_disetujui ?? 0 }} {{ $item->stock->satuan ?? '' }}
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="p-5 space-y-3">
            ...
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4 hover:bg-[#F5E6B8]/20 transition">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        <p class="font-bold text-gray-800">{{ $item->divisi_pengaju ?? '-' }}</p>
                        <p class="text-xs text-gray-500">
                            {{ $item->total_barang_diminta ?? 0 }} barang diminta
                        </p>
                    </div>
                    <div class="text-right">
                        <p class="font-bold text-purple-700">{{ $item->total_pengajuan ?? 0 }}</p>
                        <p class="text-xs text-gray-400">pengajuan</p>
                    </div>
                </div>
            </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-[#F5E6B8]/20 transition">
                    ...
                    <td class="px-6 py-4">
                        <p class="font-semibold text-gray-800">{{ $item->nama_pengaju ?? '-' }}</p>
                        <p class="text-xs text-gray-500">{{ $item->divisi_pengaju ?? '-' }}</p>
                    </td>

                    <td class="px-6 py-4">
                        <p class="font-semibold text-gray-800">{{ $item->stock->nama_barang ?? '-' }}</p>
                        <p class="text-xs text-gray-500">{{ $item->stock->kode_barang ?? '-' }}</p>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        @if($item->status_pengajuan === 'Disetujui semua')
