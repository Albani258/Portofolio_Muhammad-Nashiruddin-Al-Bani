...
<?php
    // membuat id penumpang secara otomatis

    $getId = mysqli_query($db, "SELECT max(id_penumpang) as idTerbesar FROM tabel_penumpang");
    $data = mysqli_fetch_array($getId);
    $idpenumpang = $data['idTerbesar'];
    // mengambil angka dari id penumpang terbesar, menggunakan fungsi substr
	// dan diubah ke integer dengan (int)
	$urutan = (int) substr($idpenumpang, 4, 3);
 
	// bilangan yang diambil ini ditambah 1 untuk menentukan nomor urut berikutnya
	$urutan++;
 
	// membentuk id penumpang baru
	// perintah sprintf("%03s", $urutan); berguna untuk membuat string menjadi 3 karakter
	// misalnya perintah sprintf("%03s", 15); maka akan menghasilkan '015'
	// angka yang diambil tadi digabungkan dengan id huruf yang kita inginkan, misalnya BRG 
	$huruf = "PASS";
	$idpenumpang = $huruf . sprintf("%03s", $urutan);
?>

<body>
    <header>
        <h3>Tambah Daftar Penumpang</h3>
    </header>

    <form action="proses-tambah-penumpang.php" method="POST">

        <fieldset>

        <p>
            <label for="id">ID Penumpang </label>
            <input type="text" name="idpenumpang" required="required" value="<?php echo $idpenumpang ?>" readonly>
        </p>
        <p>
            <label for="nama">Nama Penumpang </label>
            <input type="text" name="namapenumpang" placeholder="Nama penumpang " />
        </p>
        <p>
